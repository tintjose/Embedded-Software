...
void *blur_y(void *arguments)
{
     ...
     ****************************************************************************/
    struct thread_args_y *args1 = arguments;
    
    // unsigned char *image = args->image;
    int rows = args1->rows;
    int cols = args1->cols;
    int row_s = args1->row_s;
    int row_e = args1->row_e;
    int center = args1->center;
    float *kernel = args1->kernel;
    float *tempim = args1->tempim;
    short int **smoothedim=args1->smoothedim;
        ...
        short int **smoothedim)
{
   ...
   ****************************************************************************/
   ...
   if(((*smoothedim) = (short int *) calloc(rows*cols,
    ...
    pthread_t thread1[N_T];
    int iret1[N_T];
    struct thread_args_y args1[N_T];
    ...
    for(j=0; j<N_T; j++)
    {
        ...
        args1[j].rows = rows;
        args1[j].cols = cols;
        args1[j].row_s = row_per_t*j;
        args1[j].row_e = row_per_t*(j+1);
        args1[j].center = center;
        args1[j].kernel = kernel;
        args1[j].tempim = tempim;
        args1[j].smoothedim=smoothedim;
        iret1[j] = pthread_create(&thread1[j], NULL, &blur_y, &args1[j]);
    }
